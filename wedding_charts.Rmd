...
datatable(betas_table, escape = FALSE, rownames = FALSE, options = list(pageLength = length(betas_table$Name)),
...
p_sp500_betas <- ggplot(sp500_betas_plus, aes(x = Beta, fill = Ticker)) + 
  ...
  geom_segment(aes(x= (mean(Beta[Ticker != "Other S&P 500 ticker"])), xend=   (mean(Beta[Ticker != "Other S&P 500 ticker"])), y=0, yend= 45 ), linewidth = .009, color = "deepskyblue") +
  geom_point(aes(x = (mean(Beta[Ticker != "Other S&P 500 ticker"]))       , y = 45  ), size = 2.5, color = "deepskyblue", fill = "deepskyblue", shape = 18)  +
  labs(y = "") +
  scale_y_continuous(expand = c(0, 0, .14, 0)) +
...
ticker_hist <- ggplotly(p_sp500_betas, tooltip = c("count", "Ticker")) %>% 
  style(hovertext = paste0("Name: ", betas_table$Name[2]  ,"\nTicker: ",  betas_table$Ticker[2] , "\nBeta: ", betas_table$Beta[2]  ), traces = 1  ) %>% 
  style(hovertext = paste0("Name: ", betas_table$Name[3]  ,"\nTicker: ",  betas_table$Ticker[3] , "\nBeta: ", betas_table$Beta[3]  ), traces = 2  ) %>% 
  style(hovertext = paste0("Name: ", betas_table$Name[4]  ,"\nTicker: ",  betas_table$Ticker[4] , "\nBeta: ", betas_table$Beta[4]  ), traces = 3  ) %>% 
  style(hovertext = paste0("Name: ", betas_table$Name[5]  ,"\nTicker: ",  betas_table$Ticker[5] , "\nBeta: ", betas_table$Beta[5]  ), traces = 4  ) %>% 
  style(hovertext = paste0("Name: ", betas_table$Name[6]  ,"\nTicker: ",  betas_table$Ticker[6] , "\nBeta: ", betas_table$Beta[6]  ), traces = 5  ) %>% 
  style(hovertext = paste0("Name: ", betas_table$Name[7]  ,"\nTicker: ",  betas_table$Ticker[7] , "\nBeta: ", betas_table$Beta[7]  ), traces = 6  ) %>% 
  style(hovertext = paste0("Name: ", betas_table$Name[8]  ,"\nTicker: ",  betas_table$Ticker[8] , "\nBeta: ", betas_table$Beta[8]  ), traces = 7  ) %>% 
  style(hovertext = paste0("Name: ", betas_table$Name[9]  ,"\nTicker: ",  betas_table$Ticker[9] , "\nBeta: ", betas_table$Beta[9]  ), traces = 8  ) %>% 
  style(hovertext = paste0("Name: ", betas_table$Name[10] ,"\nTicker: ",  betas_table$Ticker[10], "\nBeta: ", betas_table$Beta[10] ), traces = 9  ) %>% 
  style(hovertext = paste0("Name: ", betas_table$Name[11] ,"\nTicker: ",  betas_table$Ticker[11], "\nBeta: ", betas_table$Beta[11] ), traces = 10 )  %>% 
  style(hovertext = paste("Mean Beta of our brands:", (round(mean(sp500_betas_plus$Beta[sp500_betas_plus$Ticker != "Other S&P 500 ticker"]), 2))), traces = 23)
...
```{r, message = FALSE, warning = FALSE, echo = FALSE,  out.height= "100%", out.width = "100%"}
datatable(sp500_betas, escape = FALSE, rownames = FALSE, options = list(pageLength = length(betas_table$Name)),
            caption = htmltools::tags$caption(
    style = 'caption-side: bottom; text-align: left;',
    'Date Range: 2022-10-03 - 2024-09-20')) %>% 
  formatRound(c("Beta", "Standard Error"), digits = 2) 
```
