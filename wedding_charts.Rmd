...
What better way to show our love than with data-driven charts and analyses? Each tab above provides insights to our love using various data sources. We pulled data from our phone service providers, annual ski passes, credit card statements, and more!
...
**Each box below corresponds to a percentage of our disposable income spent on a given category (i.e. this excludes boring spending like mortgage, utilities, student loans, or car payments).**
<br>
...
The tabs below provide deeper insights using statistical modeling and analyses. Enjoy!
...
reg_table_in %>% 
  ...
  add_footnote(label = "\nPredictor variable = Daily quantity of texts sent between Neil and Karina\nOutcome Variable = Corresponding index closing price (Absolute closing value or Daily Difference closing value)\nAll models controlled for time and the interaction term between texting and time is included\n*p<.05 for individual regression coefficient",notation = "none") 
...
***Statistical Interpretation:*** This model *is* statistically significant: F(3, 407) = 519.61, p \<.01, R<sup>2</sup> = 0.79. Suggesting that, for every 1 text we send, the Dow Jones index price is expected to increase 11.40 points at day 0 (p \<.01) - however, for every day that passes, the regression coefficient (slope) is expected to *decrease* by 0.0368 points (p \<.01). The chart above shows the expected change in slope at 0, 200, 400, 600, & 800 days since we started texting. As can be seen from the chart above, after enough time, the relationship turns from positive to negative!
...
**Our spending patterns over the course of our relationship do show spending on brands and services that are generally well coordinated with the market (Betas > 1 below):**
...
datatable(betas_table, escape = FALSE, rownames = FALSE, options = list(pageLength = length(betas_table$Company)),
...
**Our Brands compared to the S&P 500** 
...
p_sp500_betas <- ggplot(sp500_betas_plus, aes(x = Beta, fill = Ticker)) + 
  ...
  scale_fill_manual(values = c( "#ff9900", "#FF6A60", "#1fbad6", "#451400", "#007cba", "#104c8c", "#000000", "#87CEEB", "#006241", "#ef4a75", "grey", "blue")) +
  geom_segment(aes(x= (mean(Beta[Ticker != "Other S&P 500 ticker"])), xend=   (mean(Beta[Ticker != "Other S&P 500 ticker"])), y=0, yend= 46 ), linewidth = .009, color = "deepskyblue") +
...
ticker_hist <- ggplotly(p_sp500_betas, tooltip = c("count", "Ticker")) %>% 
  ...
  style(hovertext = paste("Mean Beta of our companies:", (round(mean(sp500_betas_plus$Beta[sp500_betas_plus$Ticker != "Other S&P 500 ticker"]), 2))), traces = 23)
...
good_service <- ggplot(cus_spend_long, aes(x = `Month-Year`, y = `Total Customer Spending (Billions)`, color = `Spending Type` )) +
  geom_line() +
  theme_classic() +
  theme(axis.text.x = element_text(angle = 90, vjust = 0.5, hjust=1))  +
  scale_x_date(date_labels="%b %Y",date_breaks  ="1 month") +
...
good_service_plotly <- ggplotly(good_service) %>% 
  layout(xaxis = list(autorange = FALSE), yaxis = list(autorange = TRUE)) %>% 
    style(hovertext = paste0('Total Customer Spending (Billions): $', cus_spend_long$`Total Customer Spending (Billions)`[cus_spend_long$`Spending Type` == "Durable goods"], 
                 '<br>Month-Year: ', format(as.Date(cus_spend_long$`Month-Year`[cus_spend_long$`Spending Type` == "Durable goods"]), "%B-%Y"),
                 '<br>Spending Type: ', cus_spend_long$`Spending Type`[cus_spend_long$`Spending Type` == "Durable goods"]), traces = 1) %>% 
    style(hovertext = paste0('Total Customer Spending (Billions): $', cus_spend_long$`Total Customer Spending (Billions)`[cus_spend_long$`Spending Type` == "Nondurable goods"], 
                 '<br>Month-Year: ', format(as.Date(cus_spend_long$`Month-Year`[cus_spend_long$`Spending Type` == "Nondurable goods"]), "%B-%Y"),
                 '<br>Spending Type: ', cus_spend_long$`Spending Type`[cus_spend_long$`Spending Type` == "Nondurable goods"]), traces = 2) %>% 
    style(hovertext = paste0('Total Customer Spending (Billions): $', cus_spend_long$`Total Customer Spending (Billions)`[cus_spend_long$`Spending Type` == "Services"],
                 ...
                 '<br>Spending Type: ', cus_spend_long$`Spending Type`[cus_spend_long$`Spending Type` == "Services"]), traces = 3) 

good_service_plotly 
```
<sup>*Click on the chart legend to show/hide any Spending Types. Y-axis will automatically readjust.*</sup>
