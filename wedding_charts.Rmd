...
vertical_feet <- ggplot(hood_vert_graph, aes(x = Date, y =  `Vertical Ft. Gained`, color = Season)) +
  ...
  #  annotate("rect", xmin = as_date("2023-04-07"), xmax = as_date("2023-12-22"), ymin = 0.0, ymax = 16000, 
   #        alpha = .25) + 
 #   annotate("pointrange", x = as_date("2023-08-15"), y = 7500, 
  #         label = "<b>Off Season</b>") +
  theme_classic() +
  scale_y_continuous(expand = c(0, 0, .14, 0)) +
    scale_x_date(date_labels="%b %Y",date_breaks  ="1 month", expand = c(.4,0,.1,0)) +
    theme(axis.text.x = element_text(angle = 90, vjust = 0.5, hjust=1)) +
 # geom_point(aes(x = as_date(max_date_vert$Date), max(`Vertical Ft. Gained`)), fill = "blue", color = "blue", shape=18, size = 2.5) +
  # geom_point(aes(x = as_date(`We Get Engaged!`), y =  proposal_date_vert$`Vertical Ft. Gained`),color = "red", fill = "red", shape=18, size = 2.5) +
...
ggplotly(vertical_feet) %>% 
  layout(hovermode = 'x') %>% 
  #style(hovertext = paste0("The most vertical feet we have ever gained\n in a day was ", format(max(hood_vert_graph $`Vertical Ft. Gained`), big.mark=","), " feet on ", format(as_date(max_date_vert$Date), "%b %d, %Y"), "!"), traces = 5) %>%
  #style(hovertext = paste0("We got engaged on ", format(as_date(`We Get Engaged!`), "%b %d, %Y"), "!"), traces = 6) %>% 
  style(hoverinfo = "none", traces = 3:4)
...
total_runs <- ggplot(hood_runs_graph, aes(x = Date, y =  `Total Runs`, color = Season)) +
  ...
  #  annotate("rect", xmin = as_date("2024-01-01"), xmax = as_date("2024-01-05"), ymin = 0.0, ymax = 16, 
   #        alpha = .25) + 
 #   annotate("text", x = as_date("2023-08-15"), y = 8, 
 #          label = "<b>Off Season</b>") +
  theme_classic() +
  scale_y_continuous(expand = c(0, 0, .14, 0)) +
    theme(axis.text.x = element_text(angle = 90, vjust = 0.5, hjust=1))  +
   #  geom_point(aes(x = as_date(max_date_runs$Date)[1], max(`Total Runs`)), color = "blue", fill = "blue", shape=18, size = 2.5) +
    # geom_point(aes(x = as_date(max_date_runs$Date)[2], max(`Total Runs`)), color = "blue", fill = "blue", shape=18, size = 2.5) +
     #geom_point(aes(x = as_date(`We Get Engaged!`), y =  proposal_date_runs$`Total Runs`), color = "red", fill = "red", shape=18, size = 2.5) +
...
ggplotly(total_runs) %>% 
  layout(hovermode = 'x')  %>% 
#  style(hovertext = paste0("The most runs we have ever completed\n in a day was ", format(max(hood_runs_graph$`Total Runs`), big.mark=","), " runs on ", format(as_date(max_date_runs$Date[1]), "%b %d, %Y" ), " & ", format(as_date(max_date_runs$Date[2]), "%b %d, %Y"), "!"), traces = 5:6) %>% 
 #   style(hovertext = paste0("We got engaged on ", format(as_date(`We Get Engaged!`), "%b %d, %Y"), "!"), traces = 7) %>% 
  style(hoverinfo = "none", traces = 3:4)
...
reg_table <- numdffinal %>% 
  ...
  select(Name = "name", `Degrees of Freedom`, `F-value`, `p-value`, R, `R-squared`) %>% 
...
reg_table %>% 
  ...
  add_footnote(label = "\nOutcome variable = daily quanitity of texts sent between Neil and Karina",notation = "none")
...
p_sp500 <- ggplot(textcount_sp500, aes(x = n_texts, y = `Close/Last`)) +
  geom_point() + 
  geom_smooth(method = "lm", formula = y ~ x) +
  theme_classic() +
  labs(caption = paste0("Our model (Sp500 regressed on number of texts sent) was statistically significant at p <.05. \nThe single regression model (F(", mod_sum$fstatistic[2], ",",  mod_sum$fstatistic[3], ") =", round(mod_sum$fstatistic[1], 2), " p<.05) \nindicates that for every 1 text we send, the SP500 drops about ", round(abs(model$coefficients[2]), 2), " points" )) +
  theme(plot.caption = element_text(hjust=0)) +
  annotate("text", label = paste0("Equation: SP500score = ", round(model$coefficients[2],2), "(texts) + e"), x = 200, y = max(textcount_sp500$`Close/Last`))
...
p_sp500_diff <- ggplot(textcount_sp500_diff, aes(x = n_texts, y = `diff`)) +
  geom_point() + 
  geom_smooth(method = "lm", formula = y ~ x) +
  theme_classic() +
  labs(caption = paste0("Our model (Sp500 regressed on number of texts sent) was NOT statistically significant at p <.05. \nThe single regression model (F(", mod_sum$fstatistic[2], ",",  mod_sum$fstatistic[3], ") =",
                        round(mod_sum$fstatistic[1], 2), " p>.05)")) +
  theme(plot.caption = element_text(hjust=0)) +
  annotate("text", label = paste0("Equation: SP500score difference = ", round(model$coefficients[2],2), "(texts) + e"), x = 200, y = max(textcount_sp500_diff$`diff`))


ggplotly(p_sp500_diff)
```

Dow Jones scatterplot (By absolute daily closing price)
```{r, message = FALSE, warning = FALSE, echo = FALSE, fig.height= 10, fig.width = 12 }
p_dj <- ggplot(textcount_dj, aes(x = n_texts, y = `Close`)) +
  geom_point() + 
  geom_smooth(method = "lm", formula = y ~ x) +
  theme_classic() +
  labs(caption = paste0("Our model (Sp500 regressed on number of texts sent) was statistically significant at p <.05. \nThe single regression model (F(", mod_sum$fstatistic[2], ",",  mod_sum$fstatistic[3], ") =", round(mod_sum$fstatistic[1], 2), " p<.05) \nindicates that for every 1 text we send, the SP500 drops about ", round(abs(model$coefficients[2]), 2), " points" )) +
  theme(plot.caption = element_text(hjust=0)) +
  annotate("text", label = paste0("Equation: Dow Jones score = ", round(model$coefficients[2],2), "(texts) + e"), x = 80, y = max(textcount_dj$`Close`))
...
p_dj_diff <- ggplot(textcount_dj_diff, aes(x = n_texts, y = `diff`)) +
  geom_point() + 
  geom_smooth(method = "lm", formula = y ~ x) +
  theme_classic() +
  labs(caption = paste0("Our model (dj diff regressed on number of texts sent) was NOT statistically significant at p <.05. \nThe single regression model (F(", mod_sum$fstatistic[2], ",",  mod_sum$fstatistic[3], ") =",
                        round(mod_sum$fstatistic[1], 2), " p>.05)")) +
  theme(plot.caption = element_text(hjust=0)) +
  annotate("text", label = paste0("Equation: Dow Jones difference = ", round(model$coefficients[2],2), "(texts) + e"), x = 90, y = max(textcount_dj_diff$`diff`))
...
```{r, message = FALSE, warning = FALSE, echo = FALSE, fig.height= 10, fig.width = 12, fig.show='hold'}
...
#manipulateWidget::combineWidgets(Karina_fig, Neil_fig, nrow = 1)
...
```{r, rows.print=100, eval = FALSE, echo = FALSE}
#for_karina_mon %>% 
#  group_by(`User`, `Month Year`) %>% 
