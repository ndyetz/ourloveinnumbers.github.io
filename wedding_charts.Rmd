...
<center>[**Click here to go back to our wedding website**](https://www.theknot.com/us/karina-convers-and-neil-yetz-aug-2025)</center>

## Texting<br>Data {.tabset .tabset-fade .tabset-pills}

### Word<br>Cloud {.tabset .tabset-fade .tabset-pills}

**Over the course of our relationship, we texted some words more than other. See our word cloud below.**
...
**But we are not ashamed to take Easy Rider to get us to the Beginners Freestyle Park!**
...
**Each square below corresponds to a percentage of our disposable income spent on a given category (i.e. this excludes boring spending like mortgage, utilities, student loans, or car payments).**

<br>

**Besides the obvious Bars & Restaruants and Amazon spending, we were surprised to see how much we spent on Chipotle and Golf!**
...
**Karina likes to buy things and travel!**
...
**Since we first met, both the S&P 500 and Dow Jones have gone up dramatically! We decided to examine if our relationship milestones or other relationship data could be responsible.**
...
<center>![](sp500dow.png){width="80%"}</center>

## Deeper<br>Dive {.tabset .tabset-fade .tabset-pills}

***First, w*****e decided to see if our texting data is predictive of the market.**

<br>

**According to the models in the table below, when correlated to the closing daily values of the S&P 500 and Dow Jones indices (controlling for time), *our texting is predictive of the change in the absolute market price*. The relationship is positive and statistically significant.**

<br>

**However, when compared to the daily movements in the market across each index (controlling for time), our models were no longer predictive... :(**
...
reg_table <- numdffinal %>% 
  ...
  mutate(
    ...
    `Market Name` = ifelse(`Market Name` == "sp500 difference", "S&P 500 close price (Daily Change)", `Market Name`),
    `Market Name` = ifelse(`Market Name` == "Dow Jones", "Dow Jones close price (Absolute)", `Market Name`),
    `Market Name` = ifelse(`Market Name` == "Dow Jones difference", "Dow Jones close price (Daily Change)", `Market Name`)
  ) %>% 
    rename(`Unstandardized Statistical Beta` = "Unstandardized Beta")
...
***Statistical Interpretation:*** This model *is* statistically significant: F(2, 408) = 2003.30, p \<.01, R<sup>2</sup> = 0.91. Suggesting that, for every 1 text we send, the S&P 500 market price is expected to increase 0.47 points, after controlling for time. The statistical beta (slope) for our texts was significant at p \< 0.05, indicating that, after controlling for time, our texts have a *slight* influence on the market. The overall model explains 91% of the variance in the market price (Mostly explained by time, but it's fun to think our texts influence it a little bit!).
...
***Statistical Interpretation:*** This model is *not* statistically significant: F(2,408) = 0.11, p \>.05, R<sup>2</sup> \< 0.01. Our texting is *not* predictive of the S&P 500 daily difference price.
...
***Statistical Interpretation:*** This model *is* statistically significant: F(2, 408) = 750.28, p \< .01, R<sup>2</sup> = 0.79. Suggesting that, for every 1 text we send, the Dow Jones market price is expected to increase 3.49 points, after controlling for time. The statistical beta (slope) for our texts was significant at p \< 0.05, indicating that, after controlling for time, our texts have a *slight* influence on the market. The overall model explains 79% of the variance in the market price (Mostly explained by time, but it's fun to think our texts influence it a little bit!).
...
***Statistical Interpretation:*** This model is *not* statistically significant: F(2,408) = 0.04, p \>.05, R<sup>2</sup> \< 0.01. Our texting is *not* predictive of the Dow Jones daily difference price.

### Interaction Model Table and Visuals {.tabset .tabset-fade .tabset-pills}

As can be seen in the model below, in the beginning of our relationship, our texts at day 0 (The lowest, darkest line) were predictive of an increase of market values (An upwards slope). However, the more time that passes, the relationship between our texting and change in market vaues is no longer there, and eventually turns negative!

<br>

We came up with the following stories, you can pick you're favorite:

**Story 1:** When we were building our relationship, the universe wanted us to text more to establish our relationship. The better our relationship got, other positive outcomes occured, and the market was happy. Over time, as we became established, it's more important for us (Karina, especially) to focus on her work and texting could be a defferent.
...
reg_table_in <- numdffinal_in %>% 
  ...
  mutate(`Unstandardized Beta (texts*time)` = round(estimatefinal_in2$`Unstandardized Beta (texts*time)`, 4),
    ...
    rename(`Unstandardized Statistical Beta (texts)` = "Unstandardized Beta (texts)",
           `Unstandardized Statistical Beta (texts*time)` = "Unstandardized Beta (texts*time)")
